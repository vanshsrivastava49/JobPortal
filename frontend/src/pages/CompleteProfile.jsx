...
import {
  ...
  ArrowLeft, LogOut, CheckCircle, Upload, X, ChevronRight,
  Shield, Clock,
} from "lucide-react";
...
const CompleteProfile = () => {
  ...
  const overallProgress = useMemo(() => {
    const userSections = sectionDefs[user.role] || [];
  ...
  const handleSubmit = async () => {
    ...
    try {
      const fullName = user.role === "jobseeker" ? `${(form.firstName||"").trim()} ${(form.lastName||"").trim()}`.trim() : undefined;
      const fullAddress = user.role === "business" ? [form.street,form.city,form.state,form.pincode].filter(Boolean).join(", ") : undefined;
      const payload = { ...form, ...(fullName && { fullName }), ...(fullAddress && { address: fullAddress }), skills: selectedSkills };
    ...
    if (role === "jobseeker") {
      switch (currentSection) {
        ...
        case "experience": return (
          <div>
            <SectionHeader title="Work Experience" subtitle="Optional but helps recruiters assess fit" />
            <Field label="Years of Experience">
              <input type="number" name="experience" value={form.experience||""} onChange={handleChange} placeholder="e.g. 3" style={{ ...inputStyle, maxWidth:200 }} className="cp-input" />
            </Field>
          </div>
        );
  ...
  const sections = sectionDefs[user.role] || [];
      ...
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        @media (max-width: 960px) {
          ...
          .cp-action-bar { left: 0; padding: 14px 20px; }
        }
      ...
      <div className="cp-root">
              ...
              {sections.map((sec) => {
                ...
                return (
                  <button key={sec.id} onClick={() => setCurrentSection(sec.id)}
                    className={`cp-nav-item${isActive ? " active" : ""}${isComplete ? " complete" : ""}`}>
                    ...
                    <div className="cp-nav-text">
                      ...
                      <div className="cp-nav-sub">
                        ...
                        {sec.required && <span className="cp-nav-badge">Required</span>}
                      </div>
                    </div>
        ...
        <div className="cp-action-bar">
          <div className="cp-action-meta">
            ...
            {overallProgress < 100 && <span> — Fill in all required sections to save</span>}
          </div>
          <button className="cp-save-btn" onClick={handleSubmit} disabled={overallProgress < 100 || uploading}>
            <CheckCircle size={17} />
            {uploading ? "Saving…" : overallProgress < 100 ? "Complete required fields" : "Save & Complete Profile"}
          </button>
        </div>
      </div>
    </>
