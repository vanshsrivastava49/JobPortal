                ...
                return (
                  ...
                  >
                    ...
                    <span style={{ display: "flex", alignItems: "center", gap: 8 }}>
                      {skillCounts[skill] && (
                        <span className="skill-option-count">{skillCounts[skill]}</span>
                      )}
          ...
          <div className="skill-dropdown-footer">
            ...
            {hasValue && (
              <button
                ...
                onMouseDown={(e) => { e.preventDefault(); setSkillFilter(""); setOpen(false); }}
              >
                Clear ×
              </button>
            )}
...
const Jobs = () => {
  ...
  const resetFilters = () => {
    setSearchTerm(""); setSelectedLocation("All"); setSelectedType("All");
    setSelectedPay("All"); setSkillFilter(""); setPage(1); setJobs([]); setHasMore(true);
  };
  ...
  // Count how many jobs have each skill
    ...
    <>
      <style>{`
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .search-box { background: white; border-radius: 12px; padding: 8px; box-shadow: 0 20px 60px rgba(0,0,0,0.1); display: flex; gap: 8px; flex-wrap: wrap; }
        ...
        .search-input { width: 100%; padding: 14px 16px 14px 48px; font-size: 14px; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; outline: none; transition: all 0.2s; font-family: 'Inter', sans-serif; }
        .search-input:focus { border-color: #10b981; box-shadow: 0 0 0 3px rgba(16,185,129,0.1); }
        .search-select { padding: 14px 16px; font-size: 14px; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; color: #475569; cursor: pointer; outline: none; min-width: 140px; font-weight: 500; font-family: 'Inter', sans-serif; }
        .search-select:focus { border-color: #10b981; box-shadow: 0 0 0 3px rgba(16,185,129,0.1); }
        .filter-toggle-btn { display: flex; align-items: center; gap: 6px; padding: 14px 16px; font-size: 14px; font-weight: 600; color: #475569; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; cursor: pointer; font-family: 'Inter', sans-serif; transition: all 0.2s; }
        .filter-toggle-btn:hover, .filter-toggle-btn.active { border-color: #10b981; color: #10b981; background: #f0fdf4; }
        .clear-btn { padding: 14px 20px; font-size: 14px; font-weight: 600; color: #64748b; background: white; border: 1px solid #e2e8f0; border-radius: 8px; cursor: pointer; display: flex; align-items: center; gap: 6px; transition: all 0.2s; font-family: 'Inter', sans-serif; }
        .clear-btn:hover { background: #f8fafc; color: #475569; }

        /* ── Advanced filter panel ── */
        .adv-filters { max-width: 900px; margin: 12px auto 0; background: white; border: 1px solid #e2e8f0; border-radius: 12px; padding: 20px; display: flex; gap: 24px; flex-wrap: wrap; align-items: flex-start; animation: fadeUp 0.2s ease; }
        ...
        .pay-chip { padding: 6px 14px; border-radius: 100px; font-size: 13px; font-weight: 600; cursor: pointer; border: 1px solid #e2e8f0; background: #f8fafc; color: #64748b; font-family: 'Inter', sans-serif; transition: all 0.2s; }
        .pay-chip.active { background: #d1fae5; border-color: #6ee7b7; color: #065f46; }

        /* ── Skill dropdown ── */
        .skill-bar { max-width: 1200px; margin: 0 auto 24px; padding: 0 24px; display: flex; align-items: center; gap: 10px; }
        ...
        .skill-list { max-height: 220px; overflow-y: auto; padding: 6px; scrollbar-width: thin; scrollbar-color: #e2e8f0 transparent; }
        ...
        .skill-option-right { display: flex; align-items: center; gap: 8px; }
        ...
        /* Active skill tag shown after dropdown */
        .skill-active-tag {
          display: inline-flex; align-items: center; gap: 6px;
          padding: 5px 10px 5px 12px; background: #d1fae5;
          border: 1px solid #6ee7b7; border-radius: 50px;
          font-size: 12px; font-weight: 700; color: #065f46;
        }
        .skill-active-tag button { background: none; border: none; cursor: pointer; display: flex; align-items: center; color: #065f46; padding: 0; opacity: 0.7; transition: opacity 0.15s; }
        .skill-active-tag button:hover { opacity: 1; }

        /* ── Stats bar ── */
        .stats-bar { max-width: 1200px; margin: -24px auto 40px; padding: 0 24px; }
        .stats-card { background: white; border: 1px solid #e2e8f0; border-radius: 8px; padding: 16px 24px; display: flex; align-items: center; gap: 24px; flex-wrap: wrap; font-size: 14px; }
        ...
        /* ── Job Card ── */
        .job-card { background: white; border: 1px solid #e2e8f0; border-radius: 12px; padding: 24px; transition: all 0.2s; cursor: pointer; display: flex; flex-direction: column; animation: fadeUp 0.3s ease; }
        ...
        .tag-pay { padding: 4px 10px; border-radius: 6px; font-size: 11px; font-weight: 600; display: flex; align-items: center; gap: 4px; border: 1px solid transparent; }
        .job-title { font-size: 18px; font-weight: 700; color: #0f172a; margin-bottom: 16px; line-height: 1.4; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; transition: color 0.2s; }
        .job-card:hover .job-title { color: #10b981; }
        .job-company { display: flex; align-items: center; gap: 12px; margin-bottom: 16px; }
        .company-logo { width: 40px; height: 40px; background: linear-gradient(135deg, #1e293b, #0f172a); border-radius: 8px; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
        ...
        .job-skill-pill { padding: 3px 10px; border-radius: 100px; font-size: 12px; font-weight: 500; background: #f1f5f9; border: 1px solid #e2e8f0; color: #475569; cursor: pointer; transition: all 0.15s; }
        ...
        .job-rounds-toggle { display: flex; align-items: center; justify-content: space-between; padding: 10px 12px; border-radius: 8px; background: #f8fafc; border: 1px solid #e2e8f0; cursor: pointer; font-size: 13px; font-weight: 600; color: #475569; font-family: 'Inter', sans-serif; transition: all 0.2s; width: 100%; margin-bottom: 0; }
        ...
        .job-round-num { width: 22px; height: 22px; border-radius: 50%; background: #0f172a; display: flex; align-items: center; justify-content: center; font-size: 10px; font-weight: 700; color: white; flex-shrink: 0; margin-top: 1px; }
        ...
        .btn-view { flex: 1; background: #0f172a; color: white; padding: 12px 16px; border-radius: 8px; font-size: 14px; font-weight: 600; border: none; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 8px; transition: all 0.2s; text-decoration: none; font-family: 'Inter', sans-serif; }
        .btn-view:hover { background: #10b981; }
        .btn-bookmark { width: 48px; height: 48px; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; display: flex; align-items: center; justify-content: center; cursor: pointer; transition: all 0.2s; }
        ...
        .btn-action { display: inline-flex; align-items: center; justify-content: center; gap: 8px; padding: 12px 24px; background: #10b981; color: white; font-size: 14px; font-weight: 600; border: none; border-radius: 8px; cursor: pointer; transition: all 0.2s; font-family: 'Inter', sans-serif; }
        ...
        @media (max-width: 768px) {
          ...
          .skill-dropdown-trigger { min-width: 160px; }
        }
      `}</style>
      ...
      <div className="jobs-wrapper">
                ...
                <div className="adv-filter-group">
                  ...
                  <div className="pay-chips">
                    {["All", "Paid", "Unpaid"].map(p => (
                      <button key={p} className={`pay-chip${selectedPay === p ? " active" : ""}`} onClick={() => setSelectedPay(p)}>
                        {p}
                      </button>
                    ))}
                  </div>
        ...
        {allSkills.length > 0 && (
          <div className="skill-bar">
            ...
            {/* Show active skill as a removable tag */}
            {skillFilter && (
                ...
                return (
                  <div key={job._id} ref={isLast ? lastJobRef : null} className="job-card">
                    ...
                    <div className="job-meta">
                      ...
                      {rounds.length > 0 && (
                        <div className="job-meta-item"><Layers size={13} />{rounds.length} round{rounds.length !== 1 ? "s" : ""}</div>
                      )}
                      {skills.length > 0 && (
                        <div className="job-meta-item"><Code2 size={13} />{skills.length} skill{skills.length !== 1 ? "s" : ""}</div>
                      )}
                    </div>
                            ...
                            {rounds.map((r, i) => (
                              <div key={r._id || i} className="job-round-item">
                                ...
                                <div className="job-round-info">
                                  ...
                                  {r.duration && (
                                    <div className="job-round-dur"><Clock size={10} />{r.duration}</div>
                                  )}
                                </div>
                              </div>
                            ))}
